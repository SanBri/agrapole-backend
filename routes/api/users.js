...
router.put(
  ...
  [
    auth,
    [
      check("mail", "Veuillez entrer votre mot de passe").not().isEmpty(),
  ...
  async (req, res) => {
    ...
    try {
      const isMatch = checkPassword(req.params.id, password);
      ...
      isMatch.then(async (result) => {
        if (result === true) {
          ...
          await user.save();
          res.json({ msg: "L'adresse mail a bien été modifiée" });
        } else {
          return res.status(400).json({
            errors: [{ msg: result }],
          });
